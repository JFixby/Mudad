...
//	double astar[];
  ...
  public SizeStrainSymPopa(String[] labels) {
    this();
    if (labels != null) {
      if (labels.length > 1) {
        identifier = labels[0];
  ...
  public void checkCrystalliteParameters() {
    ...
    if (actualNumber < numberCrystallite) {
      for (int i = actualNumber; i < numberCrystallite; i++) {
        if (i == 0.0)
          addparameterloopField(0, new Parameter(this, getParameterString(0, i), 500,
...
/*		Parameter microstrain = (Parameter) getMicrostrainList().elementAt(index);
  ...
  public void applySymmetryRules() {
    LGIndex = SpaceGroups.getLGNumber(aphase);
    PGIndex = SpaceGroups.getPGNumberLconvention(aphase);
...
//		Misc.println("apply sysmmetry");
  }

/*	public void updateParametertoDoubleBuffering() {
		Misc.println("updating");
    FilePar filepar = getFilePar();
		if (filepar != null) {
			Misc.println(filepar.isLoadingFile());
		}
		Misc.println(isAbilitatetoRefresh);
		super.updateParametertoDoubleBuffering();
		Misc.println("updated");
	}*/

  double cryststrain[] = new double[2];

  public double[] getCrystalliteMicrostrain(int h, int k, int l) {

    double dspace = aphase.getDspacing(h, k, l);
    cryststrain[0] = getCrystallite(dspace, h, k, l);
    cryststrain[1] = getMicrostrain(dspace, h, k, l);
    return cryststrain;
  }
  ...
  public class JSizeStrainAnisoOptionsD extends JOptionsDialog {
    ...
    public JSizeStrainAnisoOptionsD(Frame parent, XRDcat obj) {
      ...
      helpFilename = new String("popahelp.txt");
      pack();
