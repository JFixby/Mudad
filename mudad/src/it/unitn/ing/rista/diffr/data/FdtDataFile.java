...
public class FdtDataFile extends InelDataFile {
  ...
  public FdtDataFile(String[] labels) {
    this();
    if (labels != null) {
      if (labels.length > 1) {
        identifier = labels[0];
...
******************************************************************}
   ...
   Pa1     = array[1..11] of real; {param?tres polynome degré n- no more used}
...
//                        312
        ...
        int index = calName.toLowerCase().lastIndexOf(".cal");
        if (index > 0)
          calName = calName.substring(0, index);
        ...
        if (calreader == null) {
          ...
          calreader = getDataBufferedInputStream(getFolder(), calibrationFile);
        }
        if (calreader == null) {
          calreader = getDataBufferedInputStream(getFilePar().getDirectory(), calibrationFile);
        }
...
// position of the peak position of standard.
            ...
            ioe.printStackTrace();
            Misc.println("Not able to load the calibration file: " + calName);
          }
        } else
          Misc.println("Not able to find the calibration file: " + calName);
...
// this must go after the second block
            ...
            if (uncalibrated)
              ...
              datafile.setCalibratedXData(j, xvalues[j] + twothetaShift);
          }
