 ...
 * @version $Revision: 1.4 $, $Date: 2006/01/19 14:45:56 $
 * @since JDK1.1
 */
...
public class XRDMLDataFile extends it.unitn.ing.rista.diffr.MultDiffrDataFile {
  ...
  public XRDMLDataFile(String[] labels) {
    this();
    if (labels != null) {
      if (labels.length > 1) {
        identifier = labels[0];
...
//              Misc.println("intensities " + ij + " : " + intensities.item(ij).toXRDcatString());
